...
export class UserService {
  public async createUser(userData: IUser): Promise<{ user: UserDocument; tokens: { accessToken: string; refreshToken: string } }> {
  ...
  public async login(email: string, password: string): Promise<{ user: UserDocument; tokens: { accessToken: string; refreshToken: string } }> {
  ...
  public async getUserById(userId: string | Types.ObjectId | number): Promise<UserDocument> {
  ...
  public async getAllUsers(
    ...
    limit: number = 10,
    role?: string
  ): Promise<{ users: UserDocument[]; totalCount: number; totalPages: number }> {
  ...
  public async updateUser(userId: string | Types.ObjectId | number, userData: Partial<IUser>): Promise<UserDocument> {
    // Find user by id or _id based on type
    const user = typeof userId === 'number'
      ? await User.findOne({ id: userId })
      : await User.findById(userId);
    ...
    const updatedUser = typeof userId === 'number'
  ...
  public async updatePassword(
    ...
    typeof userId === 'number'
      ? await User.findOneAndUpdate({ id: userId }, { password: hashedPassword })
      : await User.findByIdAndUpdate(userId, { password: hashedPassword });
  }
  
  public async deleteUser(userId: string | Types.ObjectId | number): Promise<void> {
    // Delete by numeric id if userId is a number
    const result = typeof userId === 'number'
